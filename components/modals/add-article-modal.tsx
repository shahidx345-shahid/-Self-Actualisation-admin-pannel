...
interface AddArticleModalProps {
  isOpen: boolean
  onClose: () => void
  onAdd: (article: { title: string; category: string; url: string; status: string }) => void
}

export function AddArticleModal({ isOpen, onClose, onAdd }: AddArticleModalProps) {
        ...
        <Card className="p-4 sm:p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input placeholder="Enter article description" />
            </div>
            
            <div>
              <label className="text-sm font-medium text-foreground block mb-2">Status</label>
              <select className="w-full px-3 py-2 border border-input rounded-lg bg-background text-foreground cursor-pointer">
            ...
            <div className="flex gap-2 pt-4">
              ...
              <Button className="flex-1 cursor-pointer">Add Article</Button>
            </div>
          </div>
        </Card>
      </motion.div>
